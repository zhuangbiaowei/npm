  ...
  readJson( path.resolve(where, "package.json")
    ...
    fs.readFile(wrapfile, "utf8", function (er, wrapjson) {
      ...
      Object.keys(newwrap.dependencies || {}).forEach(function (key) {
        ...
        rv.shasums[key] = distSum(newwrap.dependencies[key])
      })
      log.verbose("readDependencies returned deps", rv.dependencies)
...
function targetResolver (where, context, deps) {
  ...
  return function resolver (what, cb) {
    ...
    // shrinkwrapped deps get laid out exactly as they were wrapped.
    // contents should not settle during shipping.
    ...
    if (wrap) {
      ...
      if (wrap[name]) {
        ...
        if (parent) {
          parent.shasums = parent.shasums || {}
          parent.shasums[name] = distSum(wrap[name])
        }
      ...
      if (parent && parent.shasums && parent.shasums[name]) {
        ...
        if (want !== got) {
          ...
          if (npm.config.get("force")) {
            log.error("shrinkwrap", er)
            log.warn("shrinkwrap", "forced, continuing")
          } else return cb(er)
        }
      }
